...
> use admin
> db.createUser({"user":"USER","pwd":"PASSWORD", roles:[{role:"clusterMonitor",db:"admin"},{role:"read",db:"DATABASE NAME",db:"local"}]})
```

## Install zabbix_sender 4.0

```bash
yum install -y https://repo.zabbix.com/zabbix/4.0/rhel/7/x86_64/zabbix-sender-4.0.35-1.el7.x86_64.rpm
```
...
vim /etc/zabbix/zabbix_agentd.d/userparameter_mongodb.conf
```bash
...
## Create script mongodb-stats.sh

vim /usr/local/bin/mongodb-stats.sh
```bash
#!/bin/bash
...
## Create script zabbix-mongodb.py

vim /usr/local/bin/zabbix-mongodb.py
...
class MongoDB(object):
    ...
    def get_long_query_metrics(self):
         """get longQuery"""
         if self.__conn is None:
             self.connect()
         db_handler = self.__conn
         ss = db_handler.admin.command({ "currentOp": True,  "microsecs_running" : {"$gte" : 300000} })

         # db longQuery
         self.add_metrics('mongodb.longQuery', len(ss['inprog']))
...
if __name__ == '__main__':
    ...
    mongodb.get_long_query_metrics()
